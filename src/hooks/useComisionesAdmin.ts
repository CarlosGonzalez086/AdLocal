...
export const useComisionesAdmin = () => {
  ...
  const cargarDashboard = useCallback(async () => {
    try {
      const { data } = await comisionesAdminApi.dashboard();
      setDashboard(data.respuesta ?? null);
    } catch {
      setDashboard(null);
    }
  }, []);
    ...
    async (
      ...
      try {
        const [r, m] = await Promise.all([
          comisionesAdminApi.resumen(periodo),
          comisionesAdminApi.movimientos(page + 1, rows, comercioId, estatus),
        ]);
  ...
  const liquidar = async (comercioId: number, periodo: string) => {
    await comisionesAdminApi.liquidar(comercioId, periodo);
  };
  return {
    dashboard,
    resumen,
